...
void handleKeyboardAsync(GLFWwindow* window)
{
    ...
    if (isShiftDown(window))
    {
        ...
        if (moveCamera)
        {
            ...
            {
                // Go forward
                ::g_pFlyCamera->moveForward(CAMERA_MOVE_SPEED * 2.0);
                //::g_pPhysicEngine->vecAABBs[0]->pPhysicInfo->position = ::g_pFlyCamera->getEyeLocation();//+= CAMERA_MOVE_SPEED * 0.1;
            }
            ...
            if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
            {
                // Go back
                ::g_pFlyCamera->moveForward(-CAMERA_MOVE_SPEED * 2.0);
                //::g_pPhysicEngine->vecAABBs[0]->pPhysicInfo->position.x -= CAMERA_MOVE_SPEED * 0.1;
            }

            if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
            {
                // Go left
                ::g_pFlyCamera->moveLeftRight(CAMERA_MOVE_SPEED * 2.0);
                //::g_pPhysicEngine->vecAABBs[0]->pPhysicInfo->position.z -= CAMERA_MOVE_SPEED * 0.1;
            }
    ...
    //            ::g_cameraEye.y -= CAMERA_MOVE_SPEED;
                ::g_pFlyCamera->moveUpDown(CAMERA_MOVE_SPEED * 2.0);
                //::g_pPhysicEngine->vecAABBs[0]->pPhysicInfo->position.y += CAMERA_MOVE_SPEED * 0.1;
            }
