...
//	this->m_eye += (this->m_target * scaledDistance);
	...
	::g_pPhysicEngine->vecAABBs[0]->pPhysicInfo->position.x += (this->m_target.x * scaledDistance);
	::g_pPhysicEngine->vecAABBs[0]->pPhysicInfo->position.z += (this->m_target.z * scaledDistance);
...
//	this->m_eye += glm::vec3(scaledDistance, 0.0f, 0.0f);
	...
	::g_pPhysicEngine->vecAABBs[0]->pPhysicInfo->position += (moveSideWaysDirection * scaledDistance);
...
void cBasicFlyCamera::moveUpDown(float distanceToMove)
{
	...
	::g_pPhysicEngine->vecAABBs[0]->pPhysicInfo->position += glm::vec3(0.0f, scaledDistance, 0.0f);
